...
function NextSessionCard({ nextSession }) {
  return (
    <div className="rounded-xl border border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-text)_4%,transparent)] px-comfy py-comfy backdrop-blur-md">
      <div className="flex items-center gap-snug text-[10px] font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">
        <CalendarClock size={12} />
        Next Session
      </div>
      {nextSession ? (
        <p className="mt-snug text-sm font-medium leading-snug text-[var(--color-text)]">
        ...
        <p className="mt-snug text-sm text-[var(--color-text-muted)]">
...
function TaskVelocityLine({ completed, total }) {
  return (
    <div className="flex items-center justify-between rounded-xl border border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-text)_4%,transparent)] px-comfy py-cozy backdrop-blur-md">
      <span className="flex items-center gap-snug text-xs font-medium text-[var(--color-text)]">
