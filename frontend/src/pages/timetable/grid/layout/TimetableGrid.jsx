...
export default function TimetableGrid({
  ...
  const { timetable, days, slots, entries, maxVersion } = workspace;
  ...
  const isCurrentSlot = (slot) =>
    nowMinutes >= toMinutes(slot.start_time) &&
  ...
  }, [canUndo, canRedo, onEditStateChange]);
            ...
            >
              ...
              {dragOverlayGeometry && (
                <div
                  ...
                  style={{
                    gridColumn: dragOverlayGeometry.gridColumn,
                    gridRow: dragOverlayGeometry.gridRow,
                  }}
                >
                  {draggingSubject && (
                    <SubjectChipContent
              ...
              activeCell
                ? (() => {
                    const startSlot = activeEntry
                      ? orderedSlots.find(
                          (slot) => slot.id === activeEntry.start_slot_id
                        ) || activeCell.slot
                      : activeCell.slot;
                    const endSlot = activeEntry
                      ? orderedSlots.find(
                          (slot) => slot.id === activeEntry.end_slot_id
                        ) || activeCell.slot
                      : activeCell.slot;

                    return `${WEEKDAY_FULL[activeCell.dayOfWeek]} · ${startSlot.start_time.slice(
                      0,
                      5
                    )}–${endSlot.end_time.slice(0, 5)}`;
                  })()
                : ""
            }
          />
