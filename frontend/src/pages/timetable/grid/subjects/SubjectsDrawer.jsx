...
export default function SubjectsDrawer({ subjects, onClose }) {
  ...
  return (
    <aside className="flex h-full w-64 shrink-0 flex-col overflow-hidden rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] shadow-[0_1px_0_0_rgba(255,255,255,0.02)_inset,0_20px_40px_-24px_rgba(0,0,0,0.6)]">
      <div className="flex shrink-0 items-center justify-between border-b border-[var(--color-border)] px-roomy py-comfy">
        <div>
        ...
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="rounded-md p-snug text-[var(--color-text-muted)] transition-colors hover:bg-[var(--color-surface-alt)] hover:text-[var(--color-text)]"
      ...
      <div className="relative flex min-h-0 flex-1 overflow-hidden p-base">
        <div
          ref={scrollRef}
          onScroll={updateScrollFades}
          className="h-full min-w-0 flex-1 overflow-y-auto rounded-xl px-tight scrollbar-hidden"
        >
          <div className="flex flex-col gap-inline">
            ...
            {subjects.length === 0 && (
              <p className="px-tight py-roomy text-center text-[12px] text-[var(--color-text-muted)]">
                No subjects yet.
              </p>
            )}
          </div>
